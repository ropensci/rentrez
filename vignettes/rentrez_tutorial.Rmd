...
year <- c(2008:2014)
papers <- sapply(year,
  function(x) entrez_search(db="pubmed", term=paste("Connectome AND", x, "[PDAT]"), retmax=0)$count,
  USE.NAMES=FALSE
)
...
## Finding cross-references and external links: `entrez_link()`:

###Cross-linked data

entrez_link() is a function that enables users to retrieve
one kind of information based on another kind of information providing that those two
kind of information are connected(linked) in NCBI database system.

Here is an example illustrating how to get the transcripts sequences
...
id. With the transcript ids, `entrez_fetch()` can be used to fetch the
sequence data on the fly.

```{r, eval = F}
library(rentrez)
entrenzID <- c('11647', '351')
fa.list <- list() ##use a list to store the fasta sequences
for( gid in entrenzID) {
    ez_link <- entrez_link(dbfrom = 'gene',
                           id = id,
                           db = 'nuccore')
    refRNAid <- ez_link$gene_nuccore_refseqrna ##here we got refseq ids corresponding to gid gene
    for(rnai in refRNAid) {
        fa <- entrez_fetch(db = 'nuccore',
                           id = rnai,
                           rettype = 'fasta')
        fa.list[[length(fa.list) + 1]] <- fa
    }
}
##fa.list now contrains the sequence data of genes of interest
str(fa.list)
```
...
```{r, multi_summ3}
date_and_cite <- extract_from_esummary(multi_summs, c("title", "pubdate", "pmcrefcount"))
knitr::kable(head(t(date_and_cite)), row.names=FALSE)
```
###Version 1.0 summary records: entrez_fetch()
