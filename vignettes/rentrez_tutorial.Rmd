...
By default, the NCBI limits users to only making 3 requests per second (and
`rentrez` enforces that limit). If this restriction makes an analysis you want to run too
slow, you can register to get an "API key". This key is associated with a
personal account at the NCBI, and allows you to make up to 10 requests per
second. After signing up for the NCBI account you can register a key in the
[account settings page](https://www.ncbi.nlm.nih.gov/account/settings/)


If you have an API key, rentrez will allow you to take advantage of it. For 
one-off cases, this is as simple as adding the `api_key` argument to a function
call. (Not these examples are not executed as part of this document, as the 
API key used here not a real one).
...
NCBI. `rentrez` makes this easy by using the value and an environment variable
,`ENTREZ_KEY` as the key for all requests you sent to the NCBI. This can be set
for a given R session using the function `set_entrez_key()`
...
help(Startup)` for description of this file)  to include your key in all
requests.
