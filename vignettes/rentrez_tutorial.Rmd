...
and a search term(`term`).If you print the returned object you get a summary of 
what it contains, and the way the NCBI's servers translated your query:
...
the buzzword "connectome" in pubmed, programmatically creating  search terms for
the `PDAT` field:
...
IDS. So, to find open-access papers discussion this gene we could look at links
in PMC:
...
pylogenetics package ape (but not executed the code block in this vignette, so
...
As the name suggests, `XML::xpathSApply()` can be used to apply a function to
...
upload
```
The NCBI sends you back some information you can use to refer to IDs you just
uploaded. In `rentrez`, that information is represented as a `web_history` object. 
...
almost limitless ways that you could put them together. 
