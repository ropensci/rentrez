...
gather and combine data from multiple NCBI databases in the comfort of a R
session or script.
...
value of 20. We can access those 20 IDs like this:
...
combine multiple such searches using the operators `AND`, `OR` and `NOT`.
...
The complete database of mesh terms is available in EUtils, so you can search
...
To get an idea of the degree to which records in the NCBI are cross-linked we can
...
To get links from all other databases we set `db` to 'all':
...
The resulting list of links takes a format
`[source_database]_[linked_database]` and each element in that list is a vector of
IDS. So, to find open-access papers discussing this gene we could look at links
in PMC:
...
Or if were interested in this genes role in diseases we could find links to OMIM:
...
firstly use `entrez_link()` to 'convert' entrez gene id to refseq transcript
ID. 
...
by setting the `cmd` argument to 'llinks', the `cmd` argument specifies an action that
elink will perform, and 'llinks' inform elink to list 'linkout' urls for the input id(s):
...
Having found the unique IDs for some records via`entrez_search` or `entrez_link()`, you may want to
go for deeper about them. The `Eutils` API has two
...
you are after, so `rentrez` provides functions to parse and summaries summary
records.
...
can use `entrez_fetch`, using the argument `rettype` to specify for the format you'd
like the record be returned in.

###Fetch DNA sequences in fasta format

Let's extend the example given in the `entrez_link()` by downloading sequences in
...
objects parsed by the `XML` package. As an example, we can check of `taxonomy`
...
Web history feature enables us to fetch a large number of entries in batch mode. Let's assume that we are studying the effect of aging on the transcriptome of human blood. We got ~500 genes which were found to be differentially expressed between the young people and the elder ones. With the entrez ids of 500 DEGs in hand, the first thing we want to do is to get the corresponding gene name/symbol of these DEGs.  We can do this without the web history feature, just put these 500 genes into a vector and feed it to the entrez_esummary or entrez_efetch to get what we want. 
```{r, fetch_without_web_hist}
##we just take the top ranked 25 aging genes reported by van den Akker et al for illuminating purpose...
aging_genes <- c('9289','3075','23208','164668','27087','6512',
		'3458','10231','9891','81563','11098','5159',
		'53637','152789','7049',
		'3002','1524','55359','154','26056','1514','2672',
		'55020','56895','3001')
res <- entrez_summary(db='gene', id = aging_genes)
symbols <- unlist(lapply(res, function(x) x$name))
```
You may find that the way mentioned above is a little bit slow, especially when the number of DEGs increases significantlly(It's common to get ~2000 DEGs in some DEG analysis, if you feed so many ids to entrez_summary without using web history, it may actually get chocked). We need a new way which allows us to upload those ids first and then do what we want accordingly. This makes the web history way...
```{r, fetch_with_web_hist}
ref <- entrez_post(db='gene',id=aging_genes)
##get summary once for all
res <- entrez_summary(db='gene' ,web_history=ref)

##or, we can fetch 10 entries each time
nfetch <- 10
if(length(aging_genes) %% nfetch == 0)
	nround= length(aging_genes) %/% nfetch else
	nround = length(aging_genes) %/% nfetch + 1
res <- vector(mode='list')
for(i in 1:nround ) {
      start <- (i-1)*nfetch + 1 - 1 ##note that retstart starts with ZERO
      if(i == nround)
       rtmax <- length(aging_genes) %% nfetch  else
       rtmax <- nfetch
      #print(paste0(start,"-",rtmax))
      resi <- entrez_summary(db='gene', web_history=ref,
                             retstart=start, retmax=rtmax)
      res <- c(res, resi)
      }
symbols <- unlist(lapply(res, function(x) x$name))
```
In the code above, we upload those query entries first with entrez_post, which returns a web_history object serving as a key for us to refer to. Following this is the web_histroy version esummary which accepts a web_history object. Querying in this way benifits both the network loads and the NCBI eutils web server loads as we upload once for all. 

There is another feature in the web history version code-job segmentation-which means that we break the 25 DEGs into 3 pieces(that is, 10 for each loop body) in the loop command. This ensures that the query process inside each loop body executes effeciently when the total input is large.
