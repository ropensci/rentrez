...
To make the most of the NCBI's data, you need to know a little about what data
the is avaliable, where it is sotred and how to find it.  `rentrez` gives you a
few tools that help you to get to know your way around the
NCBI's databases. 
...
entrez_dbs()
```
You can learn more more about each of those database using a set of
...
database (which contains for raw data from sequencing projects):
...
```
The object return by a search acts like a list, and you can get a summary of 
...
Those IDs are the most important thing returned by `entrez_search` because they
allow you to fetch those records, gather summary data about them or find
cross-linked records in other databases. We access the IDs as a vector using the
`$` operator:
...
Narrow our focus to only those records that have been added recently (using the colon to
...
the most basic case we need to provide an ID (`id`), the database from which ID comes (`dbfrom`)
and the name of a database in which to find linked records (`db`). If we set
this last argument to 'all' we can find links in multiple database:
...
find open access publications associated with gene we could get linked records
in Pubmed Central:
...
to use the `db` argument to narrow the focus of your search.

For instance, we could find RNA transcripts associated with the Amyloid Beta Precursor 
gene. Transcript sequences are stored in the nucleotide database (often referred
...
this document the first paper linked toA myloid Beta Precursor  had a unique ID of
...
Each element in that list contains a Url for information about this paper:
...
itself is probably the most useful. For that reason `rentrez` provides the
function `linkout_urls` to make extracting just the urls easy:
...
If we wanted to use these sequences in some other application you can write them 
to file:
...
[XPath expressions](https://en.wikipedia.org/wiki/XPath) along with the functions 
`XML::xpathSApply` or the extraction operator `[` to get specific parts of the
file. For instance, we can get the scientific name of each taxon in _T.
thermophila_'s lineage by specifying an path through the XML
...
each node in an XML object. A particularly useful function to apply is `XML::xmlValue`, 
...
This tutorial has introduced you to the core function of `rentrez`, there are
